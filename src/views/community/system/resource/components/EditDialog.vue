			  ...
			  <vxe-form ref="xForm" :data="formData" :rules="formRules" title-align="right" title-width="100" @submit="onFormSubmit">
				  <vxe-form-item title="父节点" field="parentText" span="24"  :item-render="{}">
					  ...
					  <vxe-pulldown ref="parentDropdown" transfer>
						  <template #default>
								<vxe-input v-model="formData.parentText" disabled readonly placeholder="" @click="clickParentField" @suffix-click="clickParentField"></vxe-input>
						  </template>
							<template #dropdown>
								<vxe-table
								  border
								  ...
								  size="mini"
								  :data="treeData">
								  <vxe-table-column type="radio" tree-node title="menu.field.name"></vxe-table-column>
								</vxe-table>
							</template>
					  </vxe-pulldown>
				  </vxe-form-item>
	...
	export default {
		data(){
			var validateId = (rule, value, callback) => {
				...
				if(!pattern.test(this.formData.newId)){
					return new Error(" 2")
				}
				let params = {id: this.formData.newId};
				...
				if(this.idExist){
					return new Error("this.$t('menu.messages.idvalidate2')")
				}
      		};
			return {
				visible : false,
		...
		methods : {
			...
			show (currentNode) {
				...
				if(currentNode&&currentNode.id != 0){
					this.formData.id = currentNode.id
					this.formData.name = currentNode.name
					this.formData.url = currentNode.url
					this.formData.parentId= currentNode.parentId
					this.formData.parentText = currentNode.parentText
					this.formData.resourceType = currentNode.resourceType+''
					this.formData.weight = currentNode.weight
					this.formData.status = currentNode.status+''
			...
			onFormSubmit() {
				...
				menu.update(params).then(response =>{
					 //提示信息
			...
			onSelectionChange({
				...
				this.formData.parentText = row.displayName
